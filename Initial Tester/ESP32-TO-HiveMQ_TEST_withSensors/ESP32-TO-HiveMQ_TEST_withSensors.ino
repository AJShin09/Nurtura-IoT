...
const int PUMP_TIMEOUT_MS = 10000;    // pump timer
const float CALIBRATION_FACTOR = 7.5; // calibrate water flow sensor
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        ...
        case MQTT_EVENT_DATA: {
            ...
            // Check for "water" command on "nurtura/rack/control"
...
void setup() {
    ...
    if (lightMeter.begin(BH1750::CONTINUOUS_HIGH_RES_MODE)) lightFound = true;
...
void loop() {
    ...
    // read light level/2secs
    if (now - lastLightCheck > 2000) {
        lastLightCheck = now;
        if (lightFound) {
            float currentLux = lightMeter.readLightLevel();
            bool shouldBeOn = (currentLux < 5.0); //if greater than this value, turn growLight ON
